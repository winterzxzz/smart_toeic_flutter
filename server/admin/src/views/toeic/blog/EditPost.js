...
const EditPost = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await instance.put(endpoint.blog.update + '/' + id, postData)
  ...
  useEffect(() => {
    async function fetchPost() {
      const { data } = await instance.get(endpoint.blog.get + '/' + id)
      setFormData(data)
    }
    fetchPost()
  }, [id])
            ...
            <CTabContent className="p-4">
              <CTabPane visible={activeTab === 1} role="tabpanel">
                <CForm>
                  ...
                  >
                    <CButton color="secondary" variant="outline" onClick={() => handleSave()}>
                      Edit Post
                    </CButton>
                  </div>
                </CForm>
