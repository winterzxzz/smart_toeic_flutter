...
class TranscriptTestDetailCubit extends Cubit<TranscriptTestDetailState> {
  ...
  TranscriptTestDetailCubit(this._transcriptTestRepository) : super(TranscriptTestDetailState.initial());

  Future<void> getTranscriptTestDetail(String transcriptTestId) async {
    emit(state.copyWith(loadStatus: LoadStatus.loading));
    final transcriptTestDetail = await _transcriptTestRepository.getTranscriptTestDetail(transcriptTestId);
    transcriptTestDetail.fold(
      (l) => emit(state.copyWith(loadStatus: LoadStatus.failure, message: l.errors?.first.message)),
      (r) => emit(state.copyWith(loadStatus: LoadStatus.success, transcriptTests: r)),
    );
  }
}
